# widgets/dialog_odometry.py
from PyQt6.QtWidgets import QDialog
from rqt_qt_debugger.ui.ui_dialog_odometry import Ui_dialog_odometry

class DialogOdometry(QDialog, Ui_dialog_odometry):
    def __init__(self, parent=None):
        super().__init__(parent)  # 初始化Qt弹窗基类
        self.setupUi(self)        # 初始化Odometry弹窗UI
        
        # 设置窗口标题
        self.setWindowTitle("Odometry里程计输入")
        
        # 初始化所有输入框的默认值
        # Header 相关
        self.line_offset.setText("0.0")          # 时间偏移（time_offset）
        self.line_frame.setText("odom")         # 坐标系ID（frame_id）
        self.line_frame_2.setText("base_link")  # 子坐标系ID（child_frame_id）
    ...
    def get_result(self):
        """
        给主窗口返回Odometry数据，固定格式：(ROS2消息类型, 参数字典)
        适配ROS2 nav_msgs/msg/Odometry消息类型
        Odometry包含：header + child_frame_id + pose(position/orientation)
        """
        # 读取Header相关参数
        time_offset = self._str_to_float(self.line_offset.text())
        frame_id = self.line_frame.text().strip() or "odom"          # 确保frame_id有默认值
        child_frame_id = self.line_frame_2.text().strip() or "base_link"  # 确保child_frame_id有默认值
        ...
        # Odometry对应ROS2的nav_msgs/msg/Odometry消息类型
        return "nav_msgs/msg/Odometry", odometry_data